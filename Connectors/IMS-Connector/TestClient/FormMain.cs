...
namespace MedicalResearch.IdentityManagement.TestClient {
  ...
  public partial class FormMain : Form {
    ...
    private void FormMain_Load(object sender, EventArgs e) {
      try {
        this.Text = this.Text.Replace("{V}" , typeof(SubjectIdentity).Assembly.GetName().Version.ToString(3));
    ...
    private ImsConnector _CurrentConnector = null;

    private ImsConnector GetOrCreateCurrentConnector() {
      if(_CurrentConnector == null) {
        _CurrentConnector = new ImsConnector(gTxtUrl.Text, gTxtToken.Text);
      }
    ...
    private void LoadProfiles() {
      ...
      var registryNode = Microsoft.Win32.Registry.CurrentUser.CreateSubKey("SOFTWARE\\ORSCF\\ImsTestClient");
    ...
    private void InitModelClasses() {
      ...
      gCboClass.Items.Add(typeof(SubjectIdentity));
    ...
    private void SaveNew() {
      ...
      if (modelType == typeof(SubjectIdentity)) {
        var record = JsonConvert.DeserializeObject<SubjectIdentity>(gTxtJson.Text);
        success = this.GetOrCreateCurrentConnector().SubjectIdentities.AddNewSubjectIdentity(record);
      }
    ...
    private void UpdateRecord() {
      ...
      if (modelType == typeof(SubjectIdentity)) {
        var record = JsonConvert.DeserializeObject<SubjectIdentity>(gTxtJson.Text);
        success = this.GetOrCreateCurrentConnector().SubjectIdentities.UpdateSubjectIdentity(record);
      }
      else if (modelType == typeof(StudyExecutionScope)) {
        var record = JsonConvert.DeserializeObject<StudyExecutionScope>(gTxtJson.Text);
    ...
    private void Delete() {
      ...
      if (modelType == typeof(SubjectIdentity)) {
        var record = JsonConvert.DeserializeObject<SubjectIdentity>(gTxtJson.Text);
        success = this.GetOrCreateCurrentConnector().SubjectIdentities.DeleteSubjectIdentityByRecordId(record.RecordId);
      }
      else if (modelType == typeof(StudyExecutionScope)) {
        var record = JsonConvert.DeserializeObject<StudyExecutionScope>(gTxtJson.Text);
    ...
    private void LoadTable() {
      ...
      if (modelType == typeof(SubjectIdentity)) {
        gTable.DataSource = this.GetOrCreateCurrentConnector().SubjectIdentities.SearchSubjectIdentities(gTxtSearch.Text,gTxtSort.Text,int.Parse(gTxtPageNumber.Text), int.Parse(gTxtPageSize.Text)).ToList();
      }
      else if (modelType == typeof(StudyExecutionScope)) {
