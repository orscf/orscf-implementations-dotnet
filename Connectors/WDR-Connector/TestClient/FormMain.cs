...
using MedicalResearch.Workflow.WebAPI;
using System;
...
namespace MedicalResearch.Workflow.TestClient {
  ...
  public partial class FormMain : Form {
    ...
    private void FormMain_Load(object sender, EventArgs e) {
      try {
        this.Text = this.Text.Replace("{V}" , typeof(ResearchStudyDefinition).Assembly.GetName().Version.ToString(3));
    ...
    private WdrConnector _CurrentConnector = null;

    private WdrConnector GetOrCreateCurrentConnector() {
      if(_CurrentConnector == null) {
        _CurrentConnector = new WdrConnector(gTxtUrl.Text, gTxtToken.Text);
      }
    ...
    private void LoadProfiles() {
      ...
      var registryNode = Microsoft.Win32.Registry.CurrentUser.CreateSubKey("SOFTWARE\\ORSCF\\WdrTestClient");
    ...
    private void InitModelClasses() {
      ...
      gCboClass.Items.Add(typeof(ResearchStudyDefinition));
    ...
    private void SaveNew() {
      ...
      if (modelType == typeof(ResearchStudyDefinition)) {
        var record = JsonConvert.DeserializeObject<ResearchStudyDefinition>(gTxtJson.Text);
        success = this.GetOrCreateCurrentConnector().ResearchStudyDefinitions.AddNewResearchStudyDefinition(record);
      }
    ...
    private void UpdateRecord() {
      ...
      if (modelType == typeof(ResearchStudyDefinition)) {
        var record = JsonConvert.DeserializeObject<ResearchStudyDefinition>(gTxtJson.Text);
        success = this.GetOrCreateCurrentConnector().ResearchStudyDefinitions.UpdateResearchStudyDefinition(record);
      }
      //else if (modelType == typeof(Visit)) {
      //  var record = JsonConvert.DeserializeObject<Visit>(gTxtJson.Text);
    ...
    private void Delete() {
      ...
      if (modelType == typeof(ResearchStudyDefinition)) {
        var record = JsonConvert.DeserializeObject<ResearchStudyDefinition>(gTxtJson.Text);
        //success = this.GetOrCreateCurrentConnector().ResearchStudyDefinitions.DeleteResearchStudyDefinitionByResearchStudyDefinitionIdentity(record.TaskGuid);
      }
      //else if (modelType == typeof(Visit)) {
      //  var record = JsonConvert.DeserializeObject<Visit>(gTxtJson.Text);
    ...
    private void LoadTable() {
      ...
      if (modelType == typeof(ResearchStudyDefinition)) {
        gTable.DataSource = this.GetOrCreateCurrentConnector().ResearchStudyDefinitions.SearchResearchStudyDefinitions(gTxtSearch.Text,gTxtSort.Text,int.Parse(gTxtPageNumber.Text), int.Parse(gTxtPageSize.Text)).ToList();
      }
      //else if (modelType == typeof(Visit)) {
